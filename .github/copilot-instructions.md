...
## Flame Game Engine Best Practices

*   **FCS (Flame Component System)**: Prefer using components (`PositionComponent`, `SpriteComponent`, `TextComponent`) instead of rendering everything in the game class.
*   **Game Class**: Extend `FlameGame` rather than `Game` or legacy game classes.
*   **Initialization**: Perform asynchronous loading (sprites, audios, configurations) inside `onLoad()` which runs once, rather than `update()`.
*   **Game Loop**:
    *   Keep `update(double dt)` clean and performant. Do not do I/O, heavy parsing, or state management updates here.
    *   Always multiply movement or physics updates by `dt` to make them frame-rate independent (e.g., `position.x += speed * dt`).
*   **Input Handling**:
    *   Use modern `TapCallbacks`, `DragCallbacks`, and `KeyboardEvents` mixins for input handling rather than legacy versions.
    *   Make sure `HasKeyboardHandlerComponents` is added to the game class if sub-components need keyboard inputs.
*   **Component Lifecycle**:
    *   Use `add()` to insert a child component.
    *   Use `removeFromParent()` to safely remove a component from the tree.
*   **Assets**:
    *   Place assets inside `assets/images/`, `assets/audio/`, etc. Register them under the `flutter:` asset section in `pubspec.yaml`.
    *   Preload assets in `onLoad()` of the game class for smooth gameplay.
